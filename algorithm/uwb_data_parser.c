...
void uwb_data_parse_input(uint8_t data_char)
{
    static uint64_t tick = 0, last_tick = 0;
    int aoa = 0;
    float dis = 0;
    ...
    if (no_index[0] != -1 && no_index[1] != -1)
    {
        ...
        // if (sscanf((char *)uwb_data_pool, "NO(%*d). D(m): %f, A: %d,%*d", &dis, &aoa) == 2)
        {
            ...
            platform_printf("t:%lld\n", last_tick - platform_get_us_time());
        }
